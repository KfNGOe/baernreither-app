...
Object.keys(groupedBySource).forEach((key) => {
   ...
   let fileNamePath = 'data/json/' + key + '_all.json' ;   
   let json_in = fs.readFileSync(fileNamePath, 'utf8') ;
   console.log('json data read: ', json_in.length, ' bytes') ;
   let jsonJs_in_all = JSON.parse(json_in) ;
   buildDiplText(jsonJs_in_all, jsonJs_in_comp) ;
   //write html strings to files
   fileNamePath = 'data/txt/' + key + '_all_html.txt' ;    //data/txt/Bae_TB_8_all_html.txt  
...
//write html file
let fileNamePath = 'html/compRes.html' ;    //data/txt/Bae_TB_8_all_html.txt
